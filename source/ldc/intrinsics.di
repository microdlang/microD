...
pragma(LDC_intrinsic, "llvm.returnaddress")
    void* llvm_returnaddress(u32 level);
...
pragma(LDC_intrinsic, "llvm.frameaddress")
    void* llvm_frameaddress(u32 level);
...
pragma(LDC_intrinsic, "llvm.prefetch")
    void llvm_prefetch(void* ptr, u32 rw, u32 locality, u32 cachetype) pure @safe;
...
pragma(LDC_intrinsic, "llvm.readcyclecounter")
    u64 llvm_readcyclecounter() @safe;
...
pragma(LDC_intrinsic, "llvm.memset.p0i8.i#")
    void llvm_memset(T)(void* dst, u8 val, T len, bool volatile_ = false)
        if (__traits(isIntegral, T));
...
pragma(inline, true)
void llvm_memcpy(T)(void* dst, const(void)* src, T len, u32 alignment, bool volatile_ = false)
    if (__traits(isIntegral, T))
{
    if (volatile_)
...
/// This function is here to support the function signature of the pre-LLVM7.0 intrinsic.
pragma(inline, true)
void llvm_memmove(T)(void* dst, const(void)* src, T len, u32 alignment, bool volatile_ = false)
    if (__traits(isIntegral, T))
{
    if (volatile_)
...
/// This function is here to support the function signature of the pre-LLVM7.0 intrinsic.
pragma(inline, true)
void llvm_memset(T)(void* dst, u8 val, T len, u32 alignment, bool volatile_ = false)
    if (__traits(isIntegral, T))
{
    if (volatile_)
...
struct OverflowRet(T) {
    static assert((is(ucent) && is(T : cent)) || is(T : i64), T.stringof ~ " is not an integer type!");
    T result; ///
    bool overflow; ///
}
