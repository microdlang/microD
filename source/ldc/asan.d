...
void __asan_poison_memory_region(const(void*) addr, usize size);

// Unpoisons memory region [addr, addr+size) for AddressSanitizer.
// Method is NOT thread-safe in the sense that no two threads can
// (un)poison memory in the same memory region simultaneously.
void __asan_unpoison_memory_region(const(void*) addr, usize size);

// Returns 1 if the byte at addr is poisoned for AddressSanitizer.
// Otherwise returns 0.
i32 __asan_address_is_poisoned(const(void*) addr);
