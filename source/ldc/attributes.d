 ...
 * `foo` in this example:
 *     @allocSize(0) void* myAlloc(usize size);
 *     void foo() {
 *         auto p = myAlloc(100);
 *         p[0] = 1;
 *     }
 ...
 *
 * @allocSize(0) extern(C) void* malloc(usize size);
 * @allocSize(2,1) extern(C) void* reallocarray(void *ptr, usize nmemb,
 *                                              usize size);
 * @allocSize(0,1) void* my_calloc(usize element_size, usize count,
 *                                 bool irrelevant);
 * ---
 */
struct allocSize
...
/++
 ...
 + import ldc.attributes;
 +
 + @dynamicCompile i32 foo() { return 42; }
 + ---
 +/
...
/++
 ...
 + import ldc.attributes;
 +
 + @dynamicCompileConst __gshared i32 value = 1;
 +
 + @dynamicCompile i32 foo() { return value * 42; }
 + ---
 +/
...
/++
 ...
 + import ldc.attributes;
 +
 + @dynamicCompileEmit i32 foo() { return 42; }
 + ---
 +/
 ...
 * import ldc.attributes;
 *
 * @section(".mySection") i32 myGlobal;
 * ---
 */
struct section
{
 ...
 *
 * @target("no-sse")
 * void foo_nosse(float *A, float* B, float K, u32 n) {
 *     for (i32 i = 0; i < n; ++i)
 *         A[i] *= B[i] + K;
 * }
 * @target("arch=haswell")
 * void foo_haswell(float *A, float* B, float K, u32 n) {
 *     for (i32 i = 0; i < n; ++i)
 *         A[i] *= B[i] + K;
 * }
 * ---
